...
class PytorchHubModel(XAIModel):
    ...
    def _load_from_hub(self, model_name: str, **kwargs) -> torch.nn.Module:
        ...
        try:
            self.logger.info(f"Loading PyTorch Hub model '{model_name}' (pretrained={self.pretrained})")
            ...
            raise RuntimeError(f"Failed to load PyTorch Hub model '{model_name}': {str(e)}") from e
