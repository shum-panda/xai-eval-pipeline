...
class ModelRegistry:
    """Singleton registry for XAI _model classes (consistent with ExplainerRegistry)"""
    ...
    def __init__(self):
        ...
        # Auto-register built-in _model types
    ...
    def register(self, name: str, model_class: Type[XAIModel]) -> None:
        """Register a _model class with a name
        ...
        if name in self._registry:
            self.logger.warning(
                f"Overwriting existing _model registration for '{name}'"
            )

        self._registry[name] = model_class
        self.logger.info(f"Registered _model '{name}' -> {model_class.__name__}")

    def get(self, name: str) -> Type[XAIModel]:
        """Get a registered _model class
    ...
    def list_available(self) -> List[str]:
        """Get list of all registered _model names"""
        return list(self._registry.keys())

    def is_registered(self, name: str) -> bool:
        """Check if a _model name is registered"""
    ...
    def unregister(self, name: str) -> None:
        ...
        Raises:
            ValueError: If _model not registered
        """
        if name not in self._registry:
            raise ValueError(f"Model '{name}' not registered")

        del self._registry[name]
        self.logger.info(f"Unregistered _model '{name}'")

    def get_registry_info(self) -> Dict[str, str]:
        """Get information about registered models

        Returns:
            Dict mapping _model names to their class names
        """
        return {name: cls.__name__ for name, cls in self._registry.items()}

    def _register_builtin_models(self) -> None:
        """Register built-in _model types"""
        # Note: These will be registered when the actual _model classes are imported
