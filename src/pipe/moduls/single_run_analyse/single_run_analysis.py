...
class SingleRunAnalyse:
    ...
    def plot_metric_vs_correctness(
        ...
        for metric in metrics:
            ...
            if save_data and save_dir:
                ...
                boxplot_data = (
                    self.df.groupby("prediction_correct")[metric]
                    .agg(
                        [
                            "count",
                            "mean",
    ...
    def calculate_statistical_tests_for_all_metrics(self) -> pd.DataFrame:
        ...
        for metric in available_metrics:
            ...
            correct_data = self.df[self.df["prediction_correct"]][actual_col].dropna()
            incorrect_data = self.df[not self.df["prediction_correct"]][
                actual_col
            ].dropna()
            ...
            if len(correct_data) > 0 and len(incorrect_data) > 0:
                ...
                result = {
                    ...
                    "incorrect_samples": len(incorrect_data),
                    "correct_mean": (
                        correct_data.mean() if len(correct_data) > 0 else np.nan
                    ),
                    "incorrect_mean": (
                        incorrect_data.mean() if len(incorrect_data) > 0 else np.nan
                    ),
                    "correct_std": (
                        correct_data.std() if len(correct_data) > 0 else np.nan
                    ),
                    "incorrect_std": (
                        incorrect_data.std() if len(incorrect_data) > 0 else np.nan
                    ),
    ...
    def calculate_iou_distribution_stats(self) -> pd.DataFrame:
        ...
        for group_values, group_data in self.df.groupby(groupby_cols):
            ...
            correct_data = group_data[group_data["prediction_correct"]]["iou"]
            incorrect_data = group_data[not group_data["prediction_correct"]]["iou"]
    ...
    def plot_iou_histograms_by_correctness(self, save_dir: Path) -> Dict[str, Path]:
        ...
        for idx, (_, row) in enumerate(model_explainer_combinations.iterrows()):
            ...
            if len(correct_data) > 0:
                ...
                _counts_correct, _bin_edges = np.array([]), np.array([])
            ...
            if len(correct_data) > 0 or len(incorrect_data) > 0:
                combo_hist_data = {
                    ...
                    "correct_iou_median": (
                        correct_data.median() if len(correct_data) > 0 else np.nan
                    ),
                    "incorrect_iou_mean": (
                        incorrect_data.mean() if len(incorrect_data) > 0 else np.nan
                    ),
                    ...
                    "incorrect_iou_median": (
                        incorrect_data.median() if len(incorrect_data) > 0 else np.nan
                    ),
                }
            ...
            stats_line1 = (
                f"Correct: μ={correct_mean:.3f} σ={correct_std:.3f} med="
                f"{correct_median:.3f}"
            )
            stats_line2 = (
                f"Incorrect: μ={incorrect_mean:.3f} σ={incorrect_std:.3f} "
                f"med={incorrect_median:.3f}"
            )
    ...
    def plot_prediction_correctness_histograms(self, save_dir: Path) -> Dict[str, Path]:
        """
        Erstellt verbesserte Histogramme für alle Metriken aufgeteilt nach prediction
        correctness.
        ...
        metrics_to_plot = [
            m for m in self.metrics if m in self.df.columns and m != "iou"
        ]
        ...
        for metric in metrics_to_plot:
            ...
            fig.suptitle(
                f"{display_name} Distribution Analysis: Model Performance by "
            ...
            for idx, (_, row) in enumerate(model_explainer_combinations.iterrows()):
                ...
                title_main = f"{model.upper()} + {explainer.replace('_', ' ').title()}"
                stats_line1 = (
                    f"Correct: μ={correct_mean:.3f} σ={correct_std:.3f} "
                    f"med={correct_median:.3f}"
                )
                stats_line2 = (
                    f"Incorrect: μ={incorrect_mean:.3f} σ="
                    f"{incorrect_std:.3f} med={incorrect_median:.3f}"
                )
    ...
    def plot_pixel_precision_histograms_by_correctness(
        self, save_dir: Path
    ) -> Dict[str, Path]:
        """
        ...
        for idx, (_, combination) in enumerate(model_explainer_combinations.iterrows()):
            ...
            ax.set_title(
                f"{model_name} + {explainer_name}", fontsize=14, fontweight="bold"
            )
    ...
    def plot_pixel_recall_histograms_by_correctness(
        self, save_dir: Path
    ) -> Dict[str, Path]:
        """
    ...
    def plot_point_game_confusion_matrices(
        self, save_dir: Path, threshold: float = 0.5
    ) -> Dict[str, Path]:
        """
        ...
        """
        if (
            "point_game" not in self.df.columns
    ...
    with mlflow.start_run(run_name="Comprehensive XAI Meta Analysis"):
        ...
        summary_lines.extend(
            [
                ...
                f"   Unique models: {len(all_models)} "
                f"({', '.join(sorted(all_models))})",
                f"   Unique explainers: {len(all_explainers)} "
                f"({', '.join(sorted(all_explainers))})",
