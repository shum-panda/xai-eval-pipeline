...
from src.pipeline.pipeline_moduls.evaluation.base.metric_calculator import (
    MetricCalculator,
)
...
class XAIEvaluator:
    ...
    def evaluate_batch_metrics(
        ...
        if not valid_results:
            self._logger.warning(
                "No results with bounding boxes found for batch evaluation"
            )
            return [None] * len(results)

        self._logger.info(
            f"Processing {len(valid_results)} results with bounding boxes in batch"
        )
        ...
        if valid_attributions:
            ...
            try:
                ...
                self._logger.error(
                    f"Batch evaluation failed, falling back to individual: {e}"
                )
                ...
                for attr, mask in zip(valid_attributions, valid_bbox_masks):
                    try:
                        ...
                        self._logger.warning(
                            f"Individual evaluation also failed: {individual_e}"
                        )
        ...
        self._logger.debug(
            f"Batch evaluation completed: {sum(1 for m in final_metrics if m is not None)} valid metrics"
        )
    ...
    def _aggregate_metrics(
        ...
        valid_metrics = [
            m for m in metrics_list if m is not None and m.values is not None
        ]
